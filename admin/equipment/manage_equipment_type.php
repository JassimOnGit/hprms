...
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT * FROM `equipment_type_list` where id = '{$_GET['id']}'");
    if($qry->num_rows > 0){
        $res = $qry->fetch_array();
        foreach($res as $k => $v){
            if(!is_numeric($k))
...
<script>
    $(function(){
        $('#uni_modal #equipment_type-form').submit(function(e){
            e.preventDefault();
            var _this = $(this)
            $('.pop-msg').remove()
            var el = $('<div>')
            ...
            start_loader();
            $.ajax({
                url:_base_url_+"classes/Master.php?f=save_equipment_type",
