...
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT * FROM `nurse_list` where id = '{$_GET['id']}'");
    if($qry->num_rows > 0){
        $res = $qry->fetch_array();
        foreach($res as $k => $v){
            if(!is_numeric($k))
...
<div class="container-fluid">
    <form action="" id="nurse-form">
        ...
        <div class="form-group">
            ...
            <textarea rows="3" name="specialization" id="specialization" class="form-control form-control-sm rounded-0" placeholder="Write the nurse's specialization here." required><?php echo isset($specialization) ? $specialization : '' ?></textarea>
        </div>
        <div class="form-group">
...
<script>
    $(function(){
        $('#uni_modal #nurse-form').submit(function(e){
            e.preventDefault();
            var _this = $(this)
            $('.pop-msg').remove()
            var el = $('<div>')
            ...
            start_loader();
            $.ajax({
                url:_base_url_+"classes/Master.php?f=save_nurse",
